import 'package:calendar/entities/habit/habit_entry/habit_entry.entity.dart';
...
class Habit with _$Habit {
  ...
  String getHabitShortDescription(BuildContext context) {
    if (frequency == "daily") {
      return "${duration != null ? "${duration!.inMinutes} ${context.t.time_units.long.minute(n: duration!.inMinutes)}, " : ""}${context.t.habits.frequency[frequency!]}, ${context.t.habits.times_a_day(nb: numberOfTimes ?? 0)}";
    } else if (frequency == "weekly") {
      return "${duration != null ? "${duration!.inMinutes} ${context.t.time_units.long.minute(n: duration!.inMinutes)}, " : ""}${context.t.habits.frequency[frequency!]}, ${context.t.habits.times_a_week(nb: numberOfTimes ?? 0)}";
    } else if (frequency == "monthly") {
      return "${duration != null ? "${duration!.inMinutes} ${context.t.time_units.long.minute(n: duration!.inMinutes)}, " : ""}${context.t.habits.frequency[frequency!]}, ${context.t.habits.times_a_month(nb: numberOfTimes ?? 0)}";
    }
    return "";
  }
